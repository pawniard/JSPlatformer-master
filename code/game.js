...
var actorChars = {
  ...
  "=": Lava, "|": Lava, "v": Lava  
  "D": Cloud // Poison Cloud, moves and wobbles
  "z": Radiation
};
...
function Radiation(pos){
  this.basepos = this.pos = pos.plus(new Vector(1,0.5));
  this.size = new Vector (0.3,0.5);
  this.wobble = Math.random() * Math.PI *3;
}
Radiation.prototype.type = "radiation";
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches lava or a cloud and the player hasn't won
  // Player loses
  if (type == "lava" || "Cloud" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
    alert("You lose.")
  } else if (type == "coin") {
  ...
  // if the player touches the radiation their size doubles
  if (!this.actors.some(function(actor) {
          return actor.type == "radiation";
        })) {
